...
Geosciences. doi:
[10.1007/s11004-022-10036-8](https://doi.org/10.1007/s11004-022-10036-8).**
...
- Modeling the Third Law of Geography (i.e., basic configuration
  similarity (BCS) model).

- Spatial prediction.

More details of GOS models can be found in [Song
(2022)](https://doi.org/10.1007/s11004-022-10036-8).

 

## 2. Spatial prediction using GOS model

According to [Song (2022)](https://doi.org/10.1007/s11004-022-10036-8),
GOS model consists of four primary steps: (1) Characterizing
geographical configurations, (2) determining parameters for the optimal
similarity, (3) spatial prediction using GOS and uncertainty assessment,
and (4) model evaluation. The process of using `geosimilarity` package
to conduct GOS modeling is presented as follows.

### 2.1 Characterizing geographical configurations

The `geosimilarity` package contains two spatial datasets:

- `zn`: Spatial samples of Zn concentrations and explanatory variables
  at sample locations

- `grid`: Spatial grid data of explanatory variables used for the
  prediction
...
![](../reference/figures/correlation-2.png)
...
![](../reference/figures/correlation-3.png)
...
function, if you set more optional numbers to the `kappa` vector and a
higher value of the cross-validation repeat times `nrepeat`, a
\\\kappa\\ value enabling more accurate prediction will be selected, but
the computation time will be increased. You can specify the `cores`
parameter to use multiple CPU cores for parallel computing.

*The default ratio of train set to test set in
[`gos_bestkappa()`](https://ausgis.github.io/geosimilarity/reference/gos_bestkappa.md)
is `1:1`(`0.5`). You can specify the ratio of train set to test set by
`nsplit` parameter*
...
![Figure 1. Processes of determining the optimal similarity. (a) The
optional kappa is (0.01, 0.05, 0.1, 0.2, 0.5, 1) and nrepeat is 2. (b)
The optional kappa is (0.01, 0.02, …, 0.09, 0.1, 0.2, …, 1) and nrepeat
is 10.](../reference/figures/bestkappa-1.png)

**Figure 1**. Processes of determining the optimal similarity. (a) The
optional `kappa` is (0.01, 0.05, 0.1, 0.2, 0.5, 1) and `nrepeat` is 2.
(b) The optional `kappa` is (0.01, 0.02, …, 0.09, 0.1, 0.2, …, 1) and
`nrepeat` is 10.
...
![Figure 2. Geographially optimal similarity (GOS)-based prediction (a)
and uncertainty (b).](../reference/figures/gos_result-1.png)

**Figure 2**. Geographially optimal similarity (GOS)-based prediction
(a) and uncertainty (b).

In addition, the following codes can be used to plot uncertainty under
...
![Figure 3. Geographially optimal similarity (GOS)-based spatial
prediction uncertainties under different \zeta
values.](../reference/figures/uncertainty-1.png)

**Figure 3**. Geographially optimal similarity (GOS)-based spatial
prediction uncertainties under different \\\zeta\\ values.

### 2.4 Model evaluation

We can compare model accuracy of GOS with various models, such as
kriging, multivariate regression, regression kriging, random forest,
BCS, etc., as shown in [Song
(2022)](https://doi.org/10.1007/s11004-022-10036-8). Here is a simple
example of comparing modeling accuracy between BCS and GOS.
...
As a result, the MAE of BCS is 0.5158 and the MAE of GOS is 0.5089, the
RMSE of BCS is 0.6599 and the RMSE of GOS is 0.6523. Compared with BCS,
GOS reduced 1.34% of MAE and 1.15% of RMSE.
